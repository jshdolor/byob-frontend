...
class ReviewForm extends Component {
  ...
  onFinish = async (values) => {
    ...
    try {
      const apiMessage = await ProductService.postReview(this.props.product.id, request);
      this.setState({
        rating: 0,
        apiMessage,
        visible: true,
      ...
      <>
        {this.state.apiMessage ? (
          <Modal visible={this.state.visible} onOk={this.handleOk} onCancel={this.handleCancel} className='byob-popup' closable={false} footer={null}>
            <h1 className='title'>{this.state.apiMessage ? 'success' : 'error'}</h1>
        ...
        <Form onFinish={this.onFinish} className='review-form'>
          <Spin spinning={this.state.isSubmitting}>
            ...
            <Form.Item name='name'>
              <Input name='name' onChange={this.onNameChange} value={this.state.name} placeholder='Name' />
              <small name=''>If this field is blank, your review will be registered as Anonymous</small>
            </Form.Item>
            ...
            >
              <TextArea name='review' placeholder='Write a review' autoSize={{ minRows: 10, maxRows: 14 }} maxLength={400} />
            </Form.Item>
            <Form.Item>
