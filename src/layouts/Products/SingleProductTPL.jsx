...
const { Option } = Select;
import { amountPrecision, refillableOptions, defaultRefillableValue } from '~/config/app';

class SingleProductTPL extends Component {
  state = {
    total: this.props.product.price * (this.props.product.type.id === 2 ? defaultRefillableValue : 1),
  ...
  render() {
    const { quantity, total, currentPage, pageSize } = this.state;
    const { type, description, displayPrice, name, brand, id, image, average_ratings } = new ProductModel(this.props.product);

    const suggestions = this.props.suggestions.map((product) => new ProductModel(product));
    const { reviews } = this.props;

    const paginatedReviews = paginate(reviews, currentPage, pageSize);
          ...
          <Col sm={24} lg={10} className='product-side'>
            <div className='product-details'>
              ...
              <div className='rating-cont'>
                <StarRatingComponent name='productStarRating' value={average_ratings} editing={false} />
              </div>
            </div>
            <div className='product-order'>
              <div className='quantity-cont'>
                {type.id === 1 ? (
                  ...
                  <div className='input-number'>
                    <Select defaultValue={defaultRefillableValue} size='large' onChange={this.onQuantityChange}>
              ...
              <div className='order-button-cont'>
                ...
                <AddCart id={id} type={type} qty={quantity} className='add-cart-btn'>
              ...
              {!this.state.review ? (
                      ...
                      <div className='review-lists'>
                        {paginatedReviews.map((review, i) => (
                          <div key={i} className='review'>
                            ...
                            <StarRatingComponent name={i + review.name} value={review.ratings} editing={false} />
                            <p className='para'>{review.message}</p>
                          </div>
                        ))}
                      </div>
                      <PageSelect itemsCount={reviews.length} pageSize={pageSize} onPageChange={this.handlePageChange} currentPage={currentPage} />
                    </>
                  )}
                </div>
              ) : (
                <div className='write-review'>
                  ...
                  <div className='review-form'>
                    <ReviewForm closeForm={() => this.setState({ review: false })} product={this.props.product}></ReviewForm>
                  </div>
                </div>
              )}
            </div>
